...
CLASS="${CLASS:-problems.mkp.solvers.KMeansGA_MKP}"
CP="${CP:-../../out}"
...
while (($#)); do
  case "$1" in
    ...
    --k) K="$2"; shift 2 ;;
    --tourn) TOURN="$2"; shift 2 ;;
    --maxIter) MAXITER="$2"; shift 2 ;;
    --bitSample) BITSAMPLE="$2"; shift 2 ;;
    --clusterEveryG) CLUSTEREVERYG="$2"; shift 2 ;;
...
# Adiciona parâmetros específicos do K-means
COMMON_ARGS+=( --k "$K" --tourn "$TOURN" --maxIter "$MAXITER" --bitSample "$BITSAMPLE" --clusterEveryG "$CLUSTEREVERYG" )
...
# ===== Logs =====
suffix="KMeans_pop${POP}_mut${MUT}_repair${REPAIR}_k${K}_tourn${TOURN}_maxIter${MAXITER}_bitSample${BITSAMPLE}_clusterEveryG${CLUSTEREVERYG}"
if $ts_on; then
  suffix+="_ts"
...
mkdir -p "$LOGDIR"
COMMON_ARGS+=( --results-dir "${LOGDIR}" --variant "${suffix}"  --mkcbres "../../instances/mkp/mkcbres.txt" --algo "KMeans" )

echo "Rodando ${CLASS} para ${PATH_ORLIB} [instâncias ${RANGE_START}-${RANGE_END}]"
echo "Logs: ${LOGDIR}"
echo "TS ativado? $ts_on"
echo "K-means params: k=${K}, tourn=${TOURN}, maxIter=${MAXITER}, bitSample=${BITSAMPLE}, clusterEveryG=${CLUSTEREVERYG}"
