...
CLASS="${CLASS:-problems.mkp.solvers.ISGA_MKP}"
CP="${CP:-../../out}"
...
# Parâmetros específicos do ISGA
ALPHA="${ALPHA:-0.5}"
KMALE="${KMALE:-6}"
TOURNF="${TOURNF:-3}"
TOURNM="${TOURNM:-2}"
...
while (($#)); do
  case "$1" in
    ...
    --kMale|--kmale) KMALE="$2"; shift 2 ;;
    --tournF|--tf|--tF) TOURNF="$2"; shift 2 ;;
    --tournM|--tm|--tM) TOURNM="$2"; shift 2 ;;
...
# Adiciona parâmetros do ISGA
COMMON_ARGS+=( --alpha "$ALPHA" --kMale "$KMALE" --tournF "$TOURNF" --tournM "$TOURNM" )
...
# ===== Logs =====
suffix="ISGA_pop${POP}_mut${MUT}_repair${REPAIR}_alpha${ALPHA}_kMale${KMALE}_tournF${TOURNF}_tournM${TOURNM}"
if $ts_on; then
  suffix+="_ts"
...
mkdir -p "$LOGDIR"
COMMON_ARGS+=( --results-dir "${LOGDIR}" --variant "${suffix}"  --mkcbres "../../instances/mkp/mkcbres.txt" --algo "ISGA" )

echo "Rodando ${CLASS} para ${PATH_ORLIB} [instâncias ${RANGE_START}-${RANGE_END}]"
echo "Logs: ${LOGDIR}"
echo "TS ativado? $ts_on"
echo "ISGA params: alpha=${ALPHA}, kMale=${KMALE}, tournF=${TOURNF}, tournM=${TOURNM}"
